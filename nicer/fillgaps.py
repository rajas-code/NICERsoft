...
def fillgaps(x, y):
    ...
    Parameters:
    x : array of time values, which should be on a grid, but can be missing points (data gaps)
    y : array of time offset values for each x
    ...
    while gap_total != 0:
        ...
        for i in range(1, len(gaps)):
            diff = gaps[i + 1][0] - gaps[i][1]
            if diff > maxdiff:
                gap_to_impute = i + 1  # gap to the right of the largest continuous run
...
def filterfunction(data):
    ...
    if (length % 2) == 0:
        ...
        filterform = np.append(
            filterform, np.append(filterform[-1], np.flip(filterform))
        )
    filteredfourier = np.multiply(
        fourierdata.reshape(length, 1), filterform.reshape(length, 1)
    )
    # ifft
...
def filter_all(data):
    ...
    if np.any(np.isnan(data)):  # if there are gaps
        ...
        log.info(f"{[np.size(bb) for bb in filtereddatablocks]}")
...
def fill(data, gaps, gap_total, gap_num, xcoords, reverse=False):
    ...
    if pts_to_reflect is not None:
        if type(pts_to_reflect) == tuple:
            ...
            right_pts_to_reflect = right_pts_to_reflect + (
                filt_left_pts_to_refect[-1] - filt_right_pts_to_reflect[0]
            )  # match the two reflected data parts in the middle
            if (size / 2) % 2 == 0.5:
                mid_val = (left_pts_to_reflect[0] + right_pts_to_reflect[-1]) / 2
                left_pts_to_reflect = np.append(left_pts_to_reflect, mid_val)
            pts_to_reflect = np.append(
                left_pts_to_reflect, right_pts_to_reflect
            )  # combine left and right reflections
        ...
        line_to_add = np.arange(
            start_diff, end_diff, (end_diff - start_diff) / (size - 0.1)
        )
...
def fillgaps_main(datafile, savefile="NaN", plotresults=True):
    """A method to fill gaps in data and preserve data noise characteristics.
    ...
    datafile: (string) a file path ending in ".csv" to a comma delimited csv file with two columns formatted as
              ...
              or a .txt file with two columns separated by whitespace

              The first column is a time, usually in seconds
              The second column is a time offset (residual) of one clock relative to another
...
if __name__ == "__main__":
    fillgaps_main(sys.argv[1], sys.argv[2])
